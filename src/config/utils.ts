...
const configFileName = setConfigFileName();

const configPathArr = Array.from(
  new Set([
    resolveCWD('./config/config.dev.json'),
    resolveCWD(`./config/${configFileName}`),
    resolveCWD(`./${configFileName}`),
    resolveDir(`./config/${configFileName}`),
    resolveDir(`./${configFileName}`),
  ]),
);
...
export function getConfig<T extends boolean>(more?: T): T extends false ? UserConfig : ConfigArray {
  ...
  if (process.env.BO_COOKIE) {
    const cookies = process.env.BO_COOKIE.split('&').map(cookie => ({ cookie })) as any;
    isArray(config) &&
      cookies.forEach((cookie: string, i: number) => {
        if (config[i]) {
          config[i].cookie = getCookie(config[i].cookie, cookie);
          return;
        }
        config[i] = { cookie };
      });
