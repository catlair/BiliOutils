...
(async () => {
  ...
  if (commander.login) {
    if (commander.config) {
      return await scanLogin(resolve(process.cwd(), commander.config));
    }
    return await scanLogin();
  }
  if (commander.createCookie) {
    return await createCookie(commander.createCookie);
  }
  if (commander.config) {
    return await run(commander.config);
  }
})();
...
async function argTaskHandle(jobsPath: string, configs: ConfigArray) {
  if (commander.task) {
    return await runTask(configs, './bin/inputTask', commander.task);
  }
