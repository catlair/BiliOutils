...
async function sendCustomDmItem(roomId: number, num: number, msg: string[], delay: number[]) {
  for (let i = 0; i < num; i++) {
    logger.debug(`直播间【${roomId}】发送第${i + 1}条弹幕`);
    await sendDmMessageWithOptions({ roomId, message: getRandomItem(msg) });
    await apiDelay(delay[0] * 1000, delay[1] * 1000);
  }
}

async function sendCustomDm() {
  const { custom, delay } = TaskConfig.liveDm;
  if (!isNotEmpty(custom)) return false;
  for (const item of custom) {
    if (isArray(item)) {
      const [roomId, num, msg] = item;
      await sendCustomDmItem(roomId, num, msg, delay);
    } else {
      const { id, num, msg } = item;
      await sendCustomDmItem(id, num, msg, delay);
    }
  }
  return true;
}
